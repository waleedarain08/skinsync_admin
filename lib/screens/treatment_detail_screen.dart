...
class TreatmentDetailScreen extends ConsumerWidget {
  ...
  Widget _buildPrimaryInfo(BuildContext context, TreatmentModel treatment) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _infoSection(context, "Patient-Facing Display Name & Description", 
          "Display Name: ${treatment.patientDisplayName ?? 'N/A'}\n\n${treatment.shortDescription ?? 'No summary provided.'}"),
        context.verticalSpace(24),
        _infoSection(context, "Clinical Protocol & Detailed Process", treatment.description ?? "No detailed clinical information provided."),
      ],
    );
  }
  ...
  Widget _buildSessionsSection(BuildContext context, TreatmentModel treatment) {
    ...
    final totalFollowUps = treatment.sessions?.fold(0, (sum, s) => sum + s.followUps.length) ?? 0;

    return CollapsibleSection(
      title: "Sessions & Follow-Ups Setup ($totalSessions Sessions, $totalFollowUps Follow-Ups)",
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (session.followUps.isEmpty)
                        Text("No follow-ups defined for this session.", style: context.fonts.grey12w400)
                      else
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  children: [
                                    Icon(
                                      fu.type == 'virtual' ? Icons.videocam_outlined : Icons.person_outline_rounded,
                                      size: 16,
                                      color: CustomColors.purple,
                                    ),
                                    context.horizontalSpace(8),
                                    Text("Follow-Up $idx (${fu.type.toUpperCase()})", style: context.fonts.black13w600),
                                    const Spacer(),
                                    Text("Timing: ${fu.intervalValue ?? 0} ${fu.intervalUnit ?? 'days'}", style: context.fonts.grey12w400),
                                  ],
                                ),
                                context.verticalSpace(6),
                                Text("Duration: ${fu.durationValue ?? 0} ${fu.durationUnit}", style: context.fonts.black12w400),
                                if (fu.isImageRequired) ...[
                                  context.verticalSpace(4),
                                  Row(
                                    children: [
                                      const Icon(Icons.camera_alt_outlined, size: 12, color: CustomColors.green),
                                      context.horizontalSpace(6),
                                      Text("Patient image upload required", style: context.fonts.green14w600),
                                    ],
                                  ),
                                ],
                                if (fu.notes != null && fu.notes!.isNotEmpty) ...[
                                  context.verticalSpace(6),
                                  Text("Notes: ${fu.notes}", style: context.fonts.grey12w400.copyWith(fontStyle: FontStyle.italic)),
                                ],
                              ],
                            ),
                          );
  ...
  Widget _buildInstructionsSection(BuildContext context, TreatmentModel treatment) {
    return CollapsibleSection(
      ...
      content: Column(
        ...
        children: [
          ...
          context.verticalSpace(6),
          Text(treatment.preTreatmentInstructions ?? "No pre-treatment instructions specified.", style: context.fonts.grey14w400),
          if (treatment.preTreatmentAttachments != null && treatment.preTreatmentAttachments!.isNotEmpty) ...[
            context.verticalSpace(16),
  ...
  Widget _buildNotificationsSection(BuildContext context, TreatmentModel treatment) {
    final preSource = treatment.preNotificationSource == 'category' ? "Using Category Defaults" : "Using Custom Configuration";
    final postSource = treatment.postNotificationSource == 'category' ? "Using Category Defaults" : "Using Custom Configuration";
    ...
    return CollapsibleSection(
      ...
      content: Column(
        ...
        children: [
          Row(
            children: [
              Text("Pre-Treatment Notifications: ", style: context.fonts.black13w600),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(
                  color: treatment.preNotificationSource == 'category' ? Colors.blue.withValues(alpha: 0.1) : Colors.orange.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(4),
                ),
                child: Text(preSource, style: TextStyle(color: treatment.preNotificationSource == 'category' ? Colors.blue : Colors.orange, fontSize: 10, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
          context.verticalSpace(10),
          if (treatment.preNotifications.isEmpty)
            Text("No pre-treatment notifications defined.", style: context.fonts.grey12w400)
          else
            ...treatment.preNotifications.map((n) => Padding(
              padding: const EdgeInsets.only(bottom: 8.0, left: 16.0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("• ${n.title ?? 'Reminder'} (Timing: ${n.timing ?? 0} ${n.timingUnit ?? 'hours'} before)", style: context.fonts.black13w600),
          ...
          Row(
            children: [
              Text("Post-Treatment Notifications: ", style: context.fonts.black13w600),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(
                  color: treatment.postNotificationSource == 'category' ? Colors.blue.withValues(alpha: 0.1) : Colors.orange.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(4),
                ),
                child: Text(postSource, style: TextStyle(color: treatment.postNotificationSource == 'category' ? Colors.blue : Colors.orange, fontSize: 10, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
          context.verticalSpace(10),
          if (treatment.postNotifications.isEmpty)
            Text("No post-treatment notifications defined.", style: context.fonts.grey12w400)
          else
            ...treatment.postNotifications.map((n) => Padding(
              padding: const EdgeInsets.only(bottom: 8.0, left: 16.0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("• ${n.title ?? 'Aftercare'} (Timing: ${n.timing ?? 0} ${n.timingUnit ?? 'hours'} after)", style: context.fonts.black13w600),
                  Text(n.message ?? "", style: context.fonts.grey12w400),
                ],
              ),
            )).toList(),
  ...
  Widget _buildConsentSection(BuildContext context, TreatmentModel treatment) {
    ...
    return CollapsibleSection(
      ...
      content: Row(
        children: [
          const Icon(Icons.file_present_rounded, color: CustomColors.purple, size: 24),
          context.horizontalSpace(16),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                  hasCustom ? "Using Custom Consent Form" : "Using Category Default Consent Form",
                  style: context.fonts.black14w600,
                ),
                context.verticalSpace(4),
                Text(
                  hasCustom ? treatment.preTreatmentConsentForm!.name : "Standard clinic consent paperwork active.",
  ...
  Widget _buildDowntimeSection(BuildContext context, TreatmentModel treatment) {
    ...
    return CollapsibleSection(
      ...
      content: Row(
        children: [
          Icon(Icons.bedtime_outlined, color: level == 'None' ? Colors.green : Colors.orange, size: 24),
          context.horizontalSpace(16),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                if (level != 'None')
                  Text("Estimated healing time: $days Days", style: context.fonts.grey12w400),
              ],
            ),
          ),
        ],
          ...
          Row(
            children: [
              ...
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(
                  color: isCategory ? Colors.blue.withValues(alpha: 0.1) : Colors.orange.withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(4),
                ),
                child: Text(isCategory ? "Category Defaults" : "Custom Configuration", style: TextStyle(color: isCategory ? Colors.blue : Colors.orange, fontSize: 10, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
          context.verticalSpace(12),
          if (treatment.allowedRoles.isEmpty)
            Text("All staff roles authorized to perform this treatment.", style: context.fonts.grey14w400)
          else
            Wrap(
              spacing: 8,
              runSpacing: 8,
            ...
            child: Wrap(
              ...
              children: (area.subAreas ?? []).map((sub) {
                ...
                if (sub.children != null && sub.children!.isNotEmpty) {
                  final childNames = sub.children!.map((c) => c.name).join(", ");
  ...
  Widget _buildPricingSection(BuildContext context, TreatmentModel treatment) {
    return CollapsibleSection(
      ...
      content: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _detailRow(context, "Base Treatment Price", "\$${treatment.basePrice?.toStringAsFixed(2) ?? '0.00'}"),
          if (treatment.unitPrices != null && treatment.unitPrices!.isNotEmpty) ...[
            context.verticalSpace(16),
            const Divider(),
            context.verticalSpace(12),
            Text("Unit Price Overrides:", style: context.fonts.black13w600),
            context.verticalSpace(8),
            ...treatment.unitPrices!.entries.map((e) => Padding(
              padding: const EdgeInsets.only(bottom: 4.0),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(e.key.toUpperCase(), style: context.fonts.black12w400),
                  Text("\$${e.value.toStringAsFixed(2)}", style: context.fonts.black12w600),
                ],
              ),
            )),
          ],
  ...
  Widget _buildBusinessLogicSection(BuildContext context, TreatmentModel treatment) {
    return CollapsibleSection(
      ...
      content: Column(
        children: [
          _logicRow(
            ...
            treatment.useInAiSimulator ? "Enabled (Compatible)" : "Disabled (Incompatible)",
  ...
  Widget _logicRow(BuildContext context, IconData icon, String label, String value, bool isOk) {
    return Row(
      children: [
        Icon(icon, color: isOk ? CustomColors.purple : CustomColors.grey, size: 20),
