...
class _CategoryCreationDialogState extends State<CategoryCreationDialog> {
  ...
  void initState() {
    ...
    _preNotificationEntries = widget.initialPreNotifications != null
        ? List.from(widget.initialPreNotifications!.map((config) => NotificationEntry(
    ...
    _postNotificationEntries = widget.initialPostNotifications != null
        ? List.from(widget.initialPostNotifications!.map((config) => NotificationEntry(
            titleController: TextEditingController(text: config.title),
            messageController: TextEditingController(text: config.message),
            timingValueController: TextEditingController(text: config.timing?.toString()),
            timingUnit: config.timingUnit ?? 'hours',
  ...
  void dispose() {
    ...
    for (var entry in _preNotificationEntries) {
      entry.dispose();
    }
    for (var entry in _postNotificationEntries) {
      entry.dispose();
    }
  ...
  Widget _buildNotificationsBuilder(bool isPre) {
    ...
    final titleLabel = isPre ? "Pre-Treatment Notifications" : "Post-Treatment Notifications";
      ...
      children: [
        ...
        if (entries.isEmpty)
          Container(
            ...
            child: Text("No default notifications defined.", style: context.fonts.grey13w500),
          )
        else
          ListView.separated(
            shrinkWrap: true,
            ...
            onPressed: () {
              if (_nameController.text.isNotEmpty) {
                Navigator.pop(context, {
                  ...
                  'preNotifications': _preNotificationEntries.map((e) => e.toConfig()).toList(),
                  'postNotifications': _postNotificationEntries.map((e) => e.toConfig()).toList(),
